...
(function() {
  ...
  // ============================================================================
  // 1.1. CENTRALIZED EVENT LISTENER MANAGEMENT
  // ============================================================================
    ...
    remove(key) {
      ...
      if (listener) {
        try {
          ...
          console.warn('[Better Forge] Error removing event listener:', error);
        }
    ...
    cleanup() {
      this.listeners.forEach((listener, key) => {
        try {
          ...
          console.warn('[Better Forge] Error cleaning event listener:', error);
        }
      });
  ...
  function addEscKeyHandler(button) {
    try {
      ...
      eventManager.add(document, 'keydown', forgeState.escKeyHandler);
  ...
  function removeEscKeyHandler() {
    try {
      if (forgeState.escKeyHandler) {
        // The event manager will handle the removal during cleanup
  ...
  function generateEquipmentList() {
    ...
    if (!utils) {
      // Fallback to equipment database if game data is unavailable
      return window.equipmentDatabase?.ALL_EQUIPMENT || [];
    }
    ...
    // If no equipment found, fall back to equipment database
  ...
  function createForgePathVisualization(equipment, stat, targetTier, forgeSteps, isMissing = false) {
    ...
    if (isMissing) {
      ...
      // Use the same calculation method as checkIfCanForge for consistency
      const matchingItems = getMatchingInventoryItems(equipment, stat, targetTier);
      const itemsByTier = groupItemsByTier(matchingItems);
      const existingT1Equivalent = calculateT1Equivalents(itemsByTier);
  ...
  // ============================================================================
     ...
     function createEquipmentSearchBar() {
       try {
         ...
         eventManager.add(searchInput, 'blur', () => {
         ...
         eventManager.add(filterBtn, 'mouseleave', () => {
   ...
   function getDisenchantContent() {
     try {
       ...
       eventManager.add(disenchantBtn, 'mousedown', () => {
       ...
       eventManager.add(disenchantBtn, 'mouseup', () => {
         disenchantBtn.style.borderImage = 'url("https://bestiaryarena.com/_next/static/media/4-frame.a58d0c39.png") 6 fill stretch';
       });
       eventManager.add(disenchantBtn, 'mouseleave', () => {
         disenchantBtn.style.borderImage = 'url("https://bestiaryarena.com/_next/static/media/4-frame.a58d0c39.png") 6 fill stretch';
       });
       eventManager.add(disenchantBtn, 'keydown', e => {
         if (e.key === ' ' || e.key === 'Enter') {
           disenchantBtn.style.borderImage = 'url("https://bestiaryarena.com/_next/static/media/1-frame-pressed.e3fabbc5.png") 6 fill stretch';
         }
       });
       eventManager.add(disenchantBtn, 'keyup', e => {
         if (e.key === ' ' || e.key === 'Enter') {
           disenchantBtn.style.borderImage = 'url("https://bestiaryarena.com/_next/static/media/4-frame.a58d0c39.png") 6 fill stretch';
         }
       });
    ...
    try {
      ...
      equipmentTitle.textContent = 'Equipment';
         ...
         if (gameId && globalThis.state?.utils?.getEquipment) {
           try {
             ...
             item.textContent = equipmentName;
                 ...
                 eventManager.add(item, 'click', createSelectionHandler('equipment', equipmentName));
       ...
       stats.forEach(stat => {
         ...
         eventManager.add(statItem, 'click', createSelectionHandler('stats', stat.name));
  ...
  function addBetterForgeButton() {
    try {
      ...
      `;
      eventManager.add(forgeButton, 'click', () => { 
        try {
